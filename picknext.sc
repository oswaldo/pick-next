...
def initRoles() =
  ...
  if !os.exists(people) then os.makeDir.all(people)
  if !os.exists(rolesFile) then os.write.over(rolesFile, """[{"name": "mediator"}]""")
...
def askForNotAvailable(selection: List[(Role, Person)]): List[Role] =
  ...
  val availability: List[Person] =
    selection.map { case (role, person) =>
      ...
      if answer == "y" || answer == "yes" then person.copy(lastRole = Some(role), lastTime = Some(LocalDate.now()))
      else person.copy(lastRole = Some(Role("not available")), lastTime = Some(LocalDate.now()))
    }
